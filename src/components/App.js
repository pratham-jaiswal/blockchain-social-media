...
class App extends Component {
  ...
  async loadBlockchainData() {
    ...
    if(networkData) {
      ...
      const imagesCount = await engager.methods.imageCount().call()
      this.setState({ imagesCount })
      // Load images
      for (var i = imagesCount; i >= 1; i--) {
        const image = await engager.methods.images(i).call()
        this.setState({
  ...
  uploadImage = description => {
    ...
    ipfs.add(this.state.buffer, (error, result) => {
      ...
      this.state.engager.methods.uploadImage(result[0].hash, description).send({ from: this.state.account }).on('transactionHash', (hash) => {
        this.setState({ loading: false })
      })
    })
  }
  ...
  tipImageOwner(id, tipAmount) {
    this.setState({ loading: true })
    this.state.engager.methods.tipImageOwner(id).send({ from: this.state.account, value: tipAmount }).on('transactionHash', (hash) => {
      this.setState({ loading: false })
    })
  }
